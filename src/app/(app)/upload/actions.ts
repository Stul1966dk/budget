...
export async function importCsvFile(formData: FormData): Promise<ImportResult> {
  ...
  const { data: mappingsData, error: mappingsError } = await supabase
    .from("text_mappings")
    .select(
      "id, match_pattern, match_type, comment, category_id, min_amount, max_amount, display_mode, is_extraordinary, active",
    );
  ...
  const mappingRules = ((mappingsData ?? []) as TextMappingRow[]).map((m) => ({
    ...
    isExtraordinary: m.is_extraordinary,
  }));
  ...
  const toInsert = newRows.map((row) => {
    ...
    return {
      ...
      is_extraordinary: bestRule?.isExtraordinary ?? false,
    };
  });
